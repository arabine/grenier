...
/*****************************************/
/*  Allouer et initialiser une image	 */	
/*****************************************/

IMAGE *ImageAllocInit(int w, int h, unsigned int levelMax, unsigned char niveauInit)
{

	IMAGE *I;

	I=(IMAGE *)malloc(sizeof(IMAGE));
	if(!I) {printf("Erreur d'allocation memoire Image");exit(0);}
	...
	I->pixel = (unsigned char *)malloc(w*h*sizeof(unsigned char));
	if(!(I->pixel))
	{
		printf("erreur d'allocation memoire ImageMalloc");
		exit(0);
	}

	ImageClear(I,niveauInit);
